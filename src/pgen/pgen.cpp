...
void ProblemGenerator::CallProblemGenerator(ParameterInput *pin, bool is_restart) {
  ...
  } else if (pgen_fun_name.compare("turbulence") == 0) {
    Turbulence(pin, is_restart);
  } else if (pgen_fun_name.compare("hook_test") == 0) {
    HookTest(pin, is_restart);
  ...
  } else if (pgen_fun_name.compare("marching_cubes") == 0) {
    MarchingCubes(pin, is_restart);
