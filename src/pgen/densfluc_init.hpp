#ifndef SRCTERMS_DENSFLUC_INIT_HPP_
#define SRCTERMS_DENSFLUC_INIT_HPP_
//========================================================================================
// AthenaXXX astrophysical plasma code
// Copyright(C) 2020 James M. Stone <jmstone@ias.edu> and the Athena code team
// Licensed under the 3-clause BSD License (the "LICENSE")
//========================================================================================
//! \file densfluc_init.hpp
...
DensFlucInit::DensFlucInit(MeshBlockPack *pp, ParameterInput *pin) :
  ...
  pmy_pack(pp) {
  // allocate memory for logdens_fluc registers
  ...
  Kokkos::realloc(logdens_fluc, nmb, ncells3, ncells2, ncells1);
...
//----------------------------------------------------------------------------------------
//! \fn  void IncludeDensFlucInitTasks
//  \brief Includes task in the operator split task list that constructs new modes with
//  random amplitudes and phases that can be used to initialize logdens_fluc and B-fields
//  Called by MeshBlockPack::AddPhysics() function

TaskStatus DensFlucInit::InitializeLogDensFlucModes(int) {
  Mesh *pm = pmy_pack->pmesh;
  auto &indcs = pmy_pack->pmesh->mb_indcs;
  int is = indcs.is;
  int js = indcs.js;
  ...
  auto logdens_fluc_ = logdens_fluc;
  auto &size = pmy_pack->pmb->mb_size;
  int no_dir=3;
  // initialize turbulent log-density fluctuations to zero
  par_for("logdens_fluc_zero",DevExeSpace(),0,nmb-1,0,ncells3-1,0,ncells2-1,0,ncells1-1,
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    logdens_fluc_(m,k,j,i) = 0.0;
  });
  ...
  for (int n=0; n<mode_count_; n++) {
    par_for("logdens_fluc_compute", DevExeSpace(),0,nmb-1,0,ncells3-1,0,ncells2-1,0,ncells1-1,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      int direction=0; // use the 0th direction component for the density perturbations
      logdens_fluc_(m,k,j,i) += aka_.d_view(direction,n)*forc_real - akb_.d_view(direction,n)*forc_imag;
    });
  }
  ...
  if(r_in > 0.0){
    if (global_variable::my_rank == 0) {
      std::cout << "logdens_fluc_ computed." << std::endl;
      std::cout << "Now setting logdens_fluc amplitude inside r_in to zero" << std::endl;
    }
    par_for("logdens_fluc_amp",DevExeSpace(),0,nmb-1,0,ncells3-1,0,ncells2-1,0,ncells1-1,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      if (rad < r_in_) {
        ...
        logdens_fluc_(m,k,j,i) *= fac_smooth;
      }
    });
  }
...
//----------------------------------------------------------------------------------------
//! \fn Now we add the density perturbations

TaskStatus DensFlucInit::AddDensFluc(int) {
  auto &indcs = pmy_pack->pmesh->mb_indcs;
  ...
  if (pmy_pack->pionn == nullptr) {
    ...
    Real m0 = 0.0, m1 = 0.0, m2 = 0.0;

    Kokkos::parallel_reduce("dens-fluc-norm", Kokkos::RangePolicy<>(DevExeSpace(),0,nmkji),
    KOKKOS_LAMBDA(const int &idx, Real &sum_vol, Real &sum_logdens, Real &sum_logdens_sq) {
      ...
      Real logdens_fluc = logdens_fluc_(m,k,j,i);
      sum_vol += dvol;
    ...
    m2 = gm_sum[2];
#endif
    Real sigma = sqrt(m2/m0 - SQR(m1/m0));
    Real norm = amp/sigma;
    ...
    par_for("turb-dens", DevExeSpace(), 0,(pmy_pack->nmb_thispack-1),ks,ke,js,je,is,ie,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      u(m,IDN,k,j,i) *= exp(sqrt(norm)*logdens_fluc_(m,k,j,i));
    });
  }
...
#endif  // SRCTERMS_DENSFLUC_INIT_HPP_
