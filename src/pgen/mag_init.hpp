...
class MagInit {
 public:
  MagInit(MeshBlockPack *pp, ParameterInput *pin, const std::string &block_name);
  ~MagInit();
...
MagInit::MagInit(MeshBlockPack *pp, ParameterInput *pin,
  ...
  inv_beta = pin->GetOrAddReal(block_name, "inv_beta", 0.0);
  amin = pin->GetOrAddReal(block_name, "mag_amin", 10.0);
  ...
  mag_flag = pin->GetOrAddInteger(block_name, "mag_flag", 2);
