...
TurbulenceDriver::TurbulenceDriver(MeshBlockPack *pp, ParameterInput *pin) :
  ...
  // If true, store/restore the forcing array and RNG state via restart files
  // instead of recomputing forcing from t=0 on every restart. Defaults to
  // true: this avoids a known force_tmp1-overwrite bug that recomputing from
  // scratch (false) is still subject to, and correctly restores the
  // Box-Muller phase. It does NOT currently give a bit-exact continuation --
  // that requires full double precision round-tripping through the restart
  // file's serialized parameters (see ParameterInput::GetOrAddReal), which
  // was reverted (see git history) because it broke an unrelated exact
  // bit-equality assumption in the relativistic-MHD lwave1d test suite.
  // Users can still set this to false explicitly, e.g. to avoid the extra
  // restart-file storage.
  restart_forcing = pin->GetOrAddBoolean("turb_driving", "restart_forcing", true);
  ...
  // update time for the turbulence driver - we update the force field every
  // dt_turb_update
  dt_turb_update=pin->GetOrAddReal("turb_driving","dt_turb_update",tcorr/100.0);
  // We make sure that the code time is within dt_turb_thresh of an integer
  // multiple of dt_turb_update, i.e. we'll match the code time-step within
  // this value of dt_turb_update steps
  dt_turb_thresh=pin->GetOrAddReal("turb_driving","dt_turb_thresh",
                                    dt_turb_update/100.);
  // fraction of energy in solenoidal modes
  ...
  // if not specified, start driving at t=0
...
//----------------------------------------------------------------------------------------
//! \fn InitializeModes()
// \brief Initializes the turbulence driving modes, and so is only executed once at
...
TaskStatus TurbulenceDriver::UpdateForcing(Driver *pdrive, int stage) {
  ...
  if ((pm->ncycle >=1) && ((current_time < tdriv_duration) || turb_flag != 1)) {
    // Update the forcing and add momentum and energy only if the driving is continuous
    // or t < tdriv_duration
    ...
    if (x_turb_scale_height_ > 0) {
      ...
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        ...
        Real x1v = CellCenterX(i-is, nx1, x1min, x1max);
        force_(m,0,k,j,i) *=
            std::exp(-SQR(x1v-x_turb_center_)/(2*SQR(x_turb_scale_height_)));
        force_(m,1,k,j,i) *=
            std::exp(-SQR(x1v-x_turb_center_)/(2*SQR(x_turb_scale_height_)));
        force_(m,2,k,j,i) *=
            std::exp(-SQR(x1v-x_turb_center_)/(2*SQR(x_turb_scale_height_)));
    ...
    if (y_turb_scale_height_ > 0) {
      ...
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        ...
        Real x2v = CellCenterX(j-js, nx2, x2min, x2max);
        force_(m,0,k,j,i) *=
            std::exp(-SQR(x2v-y_turb_center_)/(2*SQR(y_turb_scale_height_)));
        force_(m,1,k,j,i) *=
            std::exp(-SQR(x2v-y_turb_center_)/(2*SQR(y_turb_scale_height_)));
        force_(m,2,k,j,i) *=
            std::exp(-SQR(x2v-y_turb_center_)/(2*SQR(y_turb_scale_height_)));
    ...
    if (z_turb_scale_height_ > 0) {
      ...
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        ...
        Real x3v = CellCenterX(k-ks, nx3, x3min, x3max);
        force_(m,0,k,j,i) *=
            std::exp(-SQR(x3v-z_turb_center_)/(2*SQR(z_turb_scale_height_)));
        force_(m,1,k,j,i) *=
            std::exp(-SQR(x3v-z_turb_center_)/(2*SQR(z_turb_scale_height_)));
        force_(m,2,k,j,i) *=
            std::exp(-SQR(x3v-z_turb_center_)/(2*SQR(z_turb_scale_height_)));
